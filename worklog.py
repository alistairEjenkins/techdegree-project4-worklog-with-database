...
class Worklog():
	...
	def main_menu(self):
		...
		self.main_menu_options[choice]() if self.valid_main_menu_choice(choice) else self.main_menu()
	...
	def get_name(self):
		'''Enter a name'''
		name = input(self.get_name.__doc__ + ' > ').rstrip()
		return name if self.validate_length(name, 'name') else self.get_name()
	
	def get_date(self):
		'''Enter a date (DD/MM/YYYY)'''
		try:
			date = datetime.datetime.strptime(input(self.get_date.__doc__ +
													' > ').strip(), '%d/%m/%Y')
		except ValueError:
			self.get_date()
		else:
	...
	def get_title(self):
		'''Enter a title'''
		title = input(self.get_title.__doc__ + ' > ').rstrip()
		return title if self.validate_length(title, 'title') else self.get_title()
	
	def get_time_spent(self):
		'''Enter time spent, rounded to nearest minute.'''
		try:
			time_spent = int(input(self.get_time_spent.__doc__ + ' > '))
		except ValueError:
			self.get_time_spent()
		else:
			return time_spent
	...
	def get_notes(self):
		...
		notes = (input(self.get_notes.__doc__+ ' > ').rstrip())
	...
	def search_menu(self):
		...
		for option, desc in self.search_menu_options.items():
			print('{}) {}'.format(option, desc.__doc__))
		choice = input('> ').lower().strip()
		self.search_menu_options[choice]() if self.valid_search_menu_choice(choice) else self.search_menu()
	...
	def find_from_range_of_dates(self):
		...
		self.clear_screen()
		print('Enter Start date, then End date')
		start_date, end_date = self.get_date(), self.get_date()
		
		self.choose_from_unique_field_values([entry.date for entry in
												  t.range_of_dates(start_date, end_date)], 'date') if self.valid_date_range(start_date, end_date) else self.find_from_range_of_dates() 
			
	def valid_date_range(self, start,end):
	...
	def find_from_term(self):
		...
		self.page_worklogs(t.entry_by_term(input('Please enter the term you wish to search by > ')))
	
	def choose_from_unique_field_values(self, values, field):
	
		if self.no_entries(values):
		...
		for entry in values:
			print(entry)
		if self.no_search(field): self.search_menu()
		choice = self.get_name() if field == 'name' else self.get_date()
		if not self.valid_choice(choice, values):
				self.choose_from_unique_field_values(values,field)
		self.page_worklogs(t.entry_by_name(choice)) if field == 'name' else \
			self.page_worklogs(t.entry_by_date(choice))
	
	def no_entries(self, values):
	
		return True if len(values) == 0 else False
	
	def no_search(self, field):
	
		return True if input('\n[Enter] to input your chosen {}, [r]eturn to search '
					 'menu.'.format(field)).lower().strip() == 'r' else False
	
	def valid_choice(self, choice, field):
	
		return True if choice in field else False
	...
	def page_worklogs(self, matches, index=0):
		...
		self.page_worklogs(matches, index)
	...
	def edit_entry(self, match):
		...
		self.search_menu() if self.edit_further() else self.edit_entry(match)
	
	def valid_edit_menu_choice(self, choice):
	
		return True if choice in self.edit_menu_options.keys() else False
	
	def edit_further(self):
		
		return False if input('\nDo you wish to edit the entry further? [y/N]').lower().strip()!= 'y' else True
		
	def delete_entry(self, match):
		'''Allows user to delete task entry'''

		if self.confirm_delete(input('Confirm deletion [y/N]')):
			t.delete(match)
			print('Worklog entry deleted')
		else:
			print("Deletion cancelled")
	
	def confirm_delete(self, delete):
